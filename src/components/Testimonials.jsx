...
const Testimonials = () => {
  ...
  const fetchReviews = async () => {
    try {
      const { data } = await axios.get('https://realestatelkbackend.vercel.app/api/sitereviews');
      setReviews(data);
      setLoading(false);
    } catch (error) {
  ...
  const submitReviewHandler = async (e) => {
    ...
    try {
      ...
      await axios.post('https://realestatelkbackend.vercel.app/api/sitereviews', { rating, comment }, config);
  ...
  const deleteReviewHandler = async (reviewId) => {
    ...
    if (result.isConfirmed) {
      try {
        const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
        await axios.delete(`https://realestatelkbackend.vercel.app/api/sitereviews/${reviewId}`, config);
  ...
  const updateReviewHandler = async (e, reviewId) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
      await axios.put(`https://realestatelkbackend.vercel.app/api/sitereviews/${reviewId}`, { rating: editRating, comment: editComment }, config);
