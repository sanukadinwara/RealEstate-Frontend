...
const location = useLocation();
  ...
  const handleDelete = async (id, e) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.delete(`https://realestatelkbackend.vercel.app/api/services/${id}`, config);
  ...
  const handleFavorite = async (id, e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
      const { data } = await axios.post(`https://realestatelkbackend.vercel.app/api/users/fav-services/${id}`, {}, config);
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const { data } = await axios.get('https://realestatelkbackend.vercel.app/api/services');
        setServices(data);
        setLoading(false);
      } catch (error) {
