...
const AdminDashboard = () => {
    ...
    const fetchInquiries = async () => {
      try {
        ...
        const [inqRes, propRes, statsRes] = await Promise.all([
          ...
          axios.get('http://localhost:5000/api/properties/pending', config).catch(() => ({ data: [] })),
          axios.get('http://localhost:5000/api/analytics/stats', config).catch(() => ({ data: { totalTraffic: 0, todayTraffic: 0 } }))
        ]);
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/inquiries/${id}/status`, { status: newStatus }, config);
