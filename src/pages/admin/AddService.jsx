...
import { FaPlus, FaTrash, FaBuilding, FaPhoneAlt, FaImage, FaAlignLeft, FaSpinner } from 'react-icons/fa';
...
const AddService = () => {
  ...
  const uploadFileHandler = async (e, type, index = null) => {
    ...
    try {
      ...
      if (type === 'logo') {
        ...
        setImages(newImages);
        toast.success(`Gallery image ${index + 1} uploaded!`);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('https://realestatelkbackend.vercel.app/api/services', serviceData, config);
    ...
    <div className="max-w-5xl mx-auto py-12 px-6">
      <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-12 border border-gray-100">
        ...
        <div className="mb-10 text-center">
          <h2 className="text-4xl font-black text-gray-900 mb-3">Register <span className="text-blue-600">Service Partner</span></h2>
          <p className="text-gray-500 font-medium">Add construction companies, architects, and interior designers</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              ...
              <input type="text" placeholder="e.g. Prime Builders" className={inputStyle} onChange={(e) => setName(e.target.value)} required />
            </div>
            
            <div className="space-y-2">
              <label className={labelStyle}>Category</label>
              <select className={inputStyle} onChange={(e) => setCategory(e.target.value)}>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                ...
                {logo && <img src={logo} alt="Preview" className="w-16 h-16 rounded-xl object-cover border border-gray-200" />}
              </div>
              {logo && <p className="text-xs text-green-600 font-medium">Image uploaded to Cloudinary!</p>}
            </div>

            <div className="space-y-2">
              <label className={labelStyle}><FaAlignLeft className="text-blue-500"/> Company Description</label>
              <textarea rows="4" placeholder="Tell users why they should choose this company..." className={`${inputStyle} resize-none`} onChange={(e) => setDescription(e.target.value)} required></textarea>
            </div>
          </div>
          ...
          <div className="border-t pt-8 mt-8">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-black text-gray-800 tracking-tight flex items-center gap-2">
                <FaImage className="text-blue-500"/> Project Gallery / Portfolio
              </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {images.map((img, index) => (
                <div key={index} className="p-5 bg-gray-50 rounded-2xl border border-gray-100 relative group space-y-3">
                  ...
                  {img && (
                    <div className="flex items-center gap-3 mt-2 border-t pt-3">
                      <img src={img} alt={`Preview ${index + 1}`} className="w-16 h-16 rounded-xl object-cover border border-gray-200" />
                      <p className="text-xs text-green-600 font-medium truncate">{img}</p>
                    </div>
                  )}
                </div>
              ))}
            </div>
            <p className="text-sm text-gray-500 mt-4 ml-2 font-medium">Upload images of past projects (max 10 recommended for speed).</p>
          </div>
          ...
          <div className="bg-blue-50/50 p-8 rounded-[2rem] border border-blue-100 space-y-4">
             ...
             <input type="text" placeholder="Phone Number" className={inputStyle} onChange={(e) => setPhone(e.target.value)} required />
          </div>
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-5 rounded-[1.5rem] font-black text-xl shadow-xl transition-all transform hover:-translate-y-1 active:scale-95 disabled:bg-gray-400" disabled={uploadingLogo || Object.values(uploadingGallery).some(v => v)}>
            {uploadingLogo || Object.values(uploadingGallery).some(v => v) ? 'Uploading Images...' : 'Publish Construction Company'}
          </button>
        </form>
      </div>
    </div>
