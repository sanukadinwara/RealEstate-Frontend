...
const MyProperties = () => {
  ...
  useEffect(() => {
    const fetchMyProperties = async () => {
      try {
        ...
        const { data } = await axios.get('http://localhost:5000/api/properties/myproperties', config);
  ...
  const handleFavoriteToggle = async (propertyId) => {
    ...
    try {
      ...
      const { data } = await axios.post(`http://localhost:5000/api/users/favorites/${propertyId}`, {}, config);
